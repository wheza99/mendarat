  ...
  const plans = [
    {
      ...
      price: "$99",
      period: "month",
      description: "Perfect for small Art Deco projects with neon gradient elements",
      features: [
        "Art Deco Design Templates",
        "Neon Gradient Components",
        "Basic Animations",
        "Email Support",
        "5 Projects"
      ],
    ...
    },
    {
      ...
      price: "$199",
      period: "month",
      description: "Advanced Art Deco solutions with comprehensive neon gradient features",
      features: [
        "Custom Art Deco Design",
    ...
    {
      name: "DECO ENTERPRISE",
      price: "$399",
      period: "month",
      description: "Enterprise-level Art Deco solutions with full neon gradient customization",
      features: [
        "Full Custom Design",
    ...
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-purple-300 max-w-3xl mx-auto font-light">
            Choose the perfect Art Deco neon gradient plan for your project needs
          </p>
        </div>
            ...
            <div key={index} className={`relative group ${plan.popular ? 'lg:scale-105' : ''}`}>
              ...
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20">
                  <div className="bg-gradient-to-r from-pink-600 to-orange-600 text-white px-6 py-2 rounded-lg text-sm font-semibold shadow-2xl">
                    MOST POPULAR
                  </div>
                </div>
              )}
              ...
              <div className={`bg-gradient-to-br from-${plan.color}-800/90 to-${plan.color}-900/90 backdrop-blur-xl border border-${plan.color}-400 rounded-lg p-8 shadow-2xl hover:shadow-${plan.color}-500/25 transition-all duration-500 transform hover:scale-105 h-full`}>
                ...
                {/* CTA Button */}
                <button className={`w-full py-3 px-6 bg-gradient-to-r from-${plan.color}-600 to-${plan.color}-700 hover:from-${plan.color}-500 hover:to-${plan.color}-600 text-white font-semibold rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-${plan.color}-500/25`}>
                  CHOOSE PLAN
                </button>
                
                {/* Deco Corner Elements */}
                <div className={`absolute -top-2 -left-2 w-4 h-4 border-2 border-${plan.color}-400 rounded-lg animate-deco-pulse`} style={{ animationDelay: `${index * 0.2 + 0.5}s` }}></div>
                <div className={`absolute -top-2 -right-2 w-4 h-4 border-2 border-${plan.color}-400 rounded-lg animate-deco-pulse`} style={{ animationDelay: `${index * 0.2 + 1}s` }}></div>
                <div className={`absolute -bottom-2 -left-2 w-4 h-4 border-2 border-${plan.color}-400 rounded-lg animate-deco-pulse`} style={{ animationDelay: `${index * 0.2 + 1.5}s` }}></div>
                <div className={`absolute -bottom-2 -right-2 w-4 h-4 border-2 border-${plan.color}-400 rounded-lg animate-deco-pulse`} style={{ animationDelay: `${index * 0.2 + 2}s` }}></div>
              </div>
        ...
        <div className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "ART DECO DESIGN", desc: "Classic geometric patterns", color: "purple" },
              { title: "NEON GRADIENTS", desc: "Modern color transitions", color: "pink" },
              { title: "PREMIUM SUPPORT", desc: "Expert assistance", color: "orange" }
            ].map((feature, index) => (
              <div key={index} className="relative group">
                <div className={`bg-gradient-to-br from-${feature.color}-800/90 to-${feature.color}-900/90 backdrop-blur-xl border border-${feature.color}-400 rounded-lg p-6 shadow-2xl hover:shadow-${feature.color}-500/25 transition-all duration-300 transform hover:scale-105`}>
                  <div className={`text-2xl font-bold text-${feature.color}-300 mb-2 animate-deco-pulse`} style={{ animationDelay: `${index * 0.2}s` }}>
                    {['◆', '◇', '◆'][index]}
                  </div>
                  <div className={`text-lg font-semibold text-${feature.color}-200 mb-2`}>
                    {feature.title}
                  </div>
                  <div className={`text-sm text-${feature.color}-400 font-light`}>
                    {feature.desc}
                  </div>
                </div>
              </div>
      ...
      <div className="absolute top-1/4 left-1/4 w-44 h-44">
        {Array.from({ length: 10 }, (_, i) => (
          <div
            key={`ray-${i}`}
            className="absolute w-1 h-20 bg-gradient-to-t from-purple-400 to-transparent animate-deco-ray"
            style={{
              left: '50%',
              top: '50%',
